...
const GAME_TILES = [
  {
    ...
    title: "QuizDash",
    status: "Live",
    description: "Answer fast, pick rewards, and survive the chaos."
  },
...
export function HostGamesPage() {
  ...
  const handlePickQuizDash = async () => {
    setBusy(true);
    setError(null);
    ...
    try {
      ...
      const botQuery = shouldSeedLocalBots ? "&bots=35" : "";
      navigate(`/host/${session.code}?token=${encodeURIComponent(session.hostToken)}${botQuery}`);
    } catch (createError) {
          ...
          {GAME_TILES.map((tile) => {
            ...
            return (
              <button
                ...
                onClick={isLive ? handlePickQuizDash : undefined}
              ...
              >
                ...
                <div className="mt-5">
                  {isLive ? (
                    <span className="cp-button-primary min-h-[3.8rem] min-w-[10rem] text-base font-black">{busy ? "Creating..." : "Play QuizDash"}</span>
                  ) : (
